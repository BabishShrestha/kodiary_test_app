...
final getCompanyListControllerProvider= StateNotifierProvider<CompanyController,AsyncValue>((ref) => CompanyController(companyListRepo: ref.read(getCompanyListProvider)));
final getcompanyListByNameControllerProvider= StateNotifierProvider<CompanyController,AsyncValue>((ref) => CompanyController(companyListRepo: ref.read(getCompanyListProvider)));
...
class CompanyController extends StateNotifier<AsyncValue>{
  ...
  // void updateWork(String body) async {
  //   final response = await _companyListRepo.updateWork();
  //   if (state is AsyncData && response!=null) return;
  //   if(state is AsyncError) {
  //     log('error');
  //   }
